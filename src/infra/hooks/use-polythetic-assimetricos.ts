 ...
 * ============================================================
 * Gerencia formulario para clusters de tipos diferentes — ex:
 * TEA (A: monothetic obrigatorio, B: polythetic com limiar).
 *
 * Caracteristicas:
 * - Clusters com tipos distintos (monothetic vs polythetic)
 * - Contadores com `tipo` e `satisfeito`
...
/** Contador extendido com tipo e status de satisfacao. */
export interface ClusterCounterAssimetrico extends ClusterCounter {
  /** Tipo do cluster: "monothetic" | "polythetic" */
  readonly tipo: string;
  /** Indica se o criterio deste cluster foi satisfeito */
...
export interface PolytheticAssimetricosReturn {
  ...
  /** Contadores por cluster com tipo e satisfacao */
  readonly contadores: Record<string, ClusterCounterAssimetrico>;
...
/* -----------------------------------------------------------
 * 4. REGRAS DE TIPO POR CLUSTER
 * ----------------------------------------------------------- */

/**
 * Determina o tipo de cluster com base na posicao e estrutura.
 * - Primeiro cluster = "monothetic" (obrigatorio)
 * - Demais = "polythetic" (com limiar)
 */
function inferirTipoCluster(
  _clusterId: string,
  index: number,
): "monothetic" | "polythetic" {
  return index === 0 ? "monothetic" : "polythetic";
}

/**
 * Determina o limiar baseado no tipo do cluster.
 */
function calcularLimiar(
  tipo: "monothetic" | "polythetic",
  totalSintomas: number,
): number {
  if (tipo === "monothetic") {
    // Cluster monothetic: todos os sintomas obrigatorios
    return totalSintomas;
  }
  // Cluster polythetic: metade arredondada para cima
...
export function usePolytheticAssimetricos(
  data: TranstornoDSM,
  ...
  // -- Contadores assimetricos (memoizado) ------------------------------------
  const contadores = useMemo<Record<string, ClusterCounterAssimetrico>>(() => {
    const resultado: Record<string, ClusterCounterAssimetrico> = {};

    data.clusters_sintomas.forEach((cluster, index) => {
      ...
      const tipo = inferirTipoCluster(cluster.id, index);
      const limiar = calcularLimiar(tipo, cluster.sintomas.length);
      const satisfeito = selecionados >= limiar;
