...
export default function TranstornoLinguagemAvaliacao() {
  ...
  // Tenta extrair perguntas de todos os locais possíveis, do mais rico ao mais simples
  const perguntasRaw: { id: string; texto: string }[] = (() => {
    // 1. Topo normalizado (ideal)
    const top = safeArray(data.perguntas_chave?.length ? data.perguntas_chave : data.key_questions);
    if (top.length) return top;
    // 2. super_enrichment.perguntas_chave_entrevista → { numero, texto }
    const seArr = safeArray(rawSuper?.perguntas_chave_entrevista as unknown[]);
    if (seArr.length) return seArr.map((p, i) => {
      const item = p as Record<string, unknown>;
      return { id: String(item.numero ?? i + 1), texto: String(item.texto ?? "") };
    });
    // 3. rendering.key_questions → strings simples
  ...
  return (
    <ConsultaShell
      titulo="Transtorno da Linguagem"
      subtitulo="Transtorno da Linguagem (315.32 / F80.2)"
    ...
    >
      <div className="xl:col-span-7 space-y-3">
        ...
        {hasClusters && clusters.map((cl) => (
          <AccordionSection key={cl.id} title={cl.nome || cl.id || cl.id} icon="fa-brain" color="blue"
            ...
            <Textarea value={notas.observacoes_clusters?.[cl.id] || ""}
              ...
              rows={2} placeholder={`Observações sobre ${cl.nome || cl.id || cl.id}...`} className="text-sm resize-y mt-2" />
          </AccordionSection>
        ))}
            ...
            <div className="space-y-1">
              {hasClusters ? clusters.map((cl) => (
                <div key={cl.id} className="flex justify-between text-xs">
                  <span className="text-slate-500">{cl.nome || cl.id || cl.id}</span>
