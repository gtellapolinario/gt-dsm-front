...
import { usePolytheticClusterEvaluation } from "@/hooks/evaluation-hooks";
...
export default function TDAHAvaliacao() {
  const hook = usePolytheticClusterEvaluation(data);
  const {
    identificacao,
    formValues,
    payload,
  ...
  return (
    <ConsultaShell
      titulo="TDAH"
      subtitulo="Transtorno de Déficit de Atenção/Hiperatividade (314.01 / F90.2)"
        ...
        {hasClusters && clusters.map((cl) => (
          <AccordionSection key={cl.id} title={cl.nome || cl.id} icon="fa-brain" color="blue"
            ...
            {safeArray(cl.sintomas).map((s) => (
              <SymptomItem key={s.id} sintoma={s} clusterId={cl.id}
